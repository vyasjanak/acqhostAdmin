...
public class NPCIXMLParser
{
	public HashMap<String, String> parseData(String XMLData) throws Exception
	{
		...
		if(list != null)
		{
			...
			{
				System.out.println("List item : "+list.item(i)+" ::: Node Type : "+list.item(i).getNodeType());
				if (list.item(i).getNodeType() == Node.ELEMENT_NODE) 
				{
					...
					{
						System.out.println(el.getTagName()+node.item(j).getNodeName()+" :: "+node.item(j).getTextContent());
					...
					if(el.getTagName().equalsIgnoreCase("Txn"))
					{
						...
						{
							System.out.println("....."+el.getTagName()+cElement.getNodeName()+cNodes.item(p).getNodeName()+" :: "+cNodes.item(p).getTextContent());
							rdData.put(el.getTagName()+cElement.getNodeName()+cNodes.item(p).getNodeName(), cNodes.item(p).getTextContent());
						}	
						...
						if(cElement.getTagName().equalsIgnoreCase("Resp"))
						{
							...
							if(chElement != null)
							{
								rdData.put(el.getTagName()+cElement.getTagName()+chElement.getTagName()+"Result",chElement.getAttribute("result"));
								System.out.println("....."+el.getTagName()+cElement.getTagName()+chElement.getTagName()+"Result ::: " +chElement.getAttribute("result"));
							}
						}
					}
					...
					if(el.getTagName().equalsIgnoreCase("Payer"))
					{
						...
						if (cElement != null)
						{
							...
							System.out.println(el.getTagName() + "Amount >>>>" + cElement.getAttribute("value"));
						}
					...
					else if(el.getTagName().equalsIgnoreCase("Merchant"))
					{
						...
						System.out.println(el.getTagName()+cElement.getTagName()+" >>>>" +cElement.getAttribute("tsRead"));
					}
					else if(el.getTagName().equalsIgnoreCase("VehicleDetails"))
					{
						...
						if(detailsList != null)
						{
							System.out.println("Details List Length : "+detailsList.getLength());
							
							for(int dL = 0 ; dL < detailsList.getLength(); dL++)
							{
								System.out.println("List item : "+detailsList.item(dL)+" ::: Node Type : "+detailsList.item(dL).getNodeType());
								if (detailsList.item(dL).getNodeType() == Node.ELEMENT_NODE) 
								{
									Element detailsElements = (Element) detailsList.item(dL);
									System.out.println("VehicleDetails : "+detailsElements.getTagName());
									System.out.println("Attributes : \n");
									NamedNodeMap detailsNode =  detailsElements.getAttributes();
							
									System.out.println(detailsNode.item(0).getTextContent()+" :: "+detailsNode.item(1).getTextContent());
									rdData.put(detailsNode.item(0).getTextContent(), detailsNode.item(1).getTextContent());
					...
					else if(el.getTagName().equalsIgnoreCase("Resp"))
					{
						...
						if(cElement != null)
						{
							rdData.put(el.getTagName()+cElement.getTagName()+"Result",cElement.getAttribute("result"));
							System.out.println(el.getTagName()+cElement.getTagName()+" >>>>" +cElement.getAttribute("result"));
						}
					}
						
					System.out.println("Value :: "+el.getTextContent());
		...
		System.out.println("NPCIXMLParser.java ::: HashMap Size :: "+rdData.size() +" And HashMap :: "+rdData);
		return rdData;
	}
	
	public static void main(String[] args) throws ParserConfigurationException, SAXException, IOException{
		String url = "http://npci.org/etc/schema/";
		/*String input = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><etc:ReqPay xmlns:etc=\""+url+"\">"
		...
		/*String input = "<Txn id=\"00000000001234560301\" note=\"\" orgTxnId=\"\" refId=\"\" refUrl=\"\" ts=\"2016-08-10T14:28:22\" type=\"FETCH\">"
		...
		try 
		{
/*			System.out.println(input.substring(input.indexOf("<Vehicle"),input.indexOf("</Vehicle>")+10));*/
